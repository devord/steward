...
function aliasSubstitution(
	...
	// Explicit arguments resolve **simultaneously**, in the caller's scope: an
	// `A` written in argument position means the caller's `A`, never the `A`
	// this call is about to bind. Resolving each against the half-filled map
	// made a reordering alias — `type Swap<A, B> = Inner<B, A>` — bind both
	// parameters to the first argument, so `Swap<unknown, string>` lost the
	// `unknown` and every dictionary behind it went unreported.
	const resolvedArguments = parameters.map((parameter, index) => {
		const argument = arguments_[index];
		return argument === undefined
			? null
			: resolvedSubstitutionArgument(argument, base);
	});

	for (const [index, parameter] of parameters.entries()) {
		const resolved = resolvedArguments[index];
		if (resolved !== null && resolved !== undefined) {
			next.set(parameter.name.name, resolved);
			continue;
		}
		// A default is written inside the alias, so unlike an argument it may
		// name the parameters declared before it — and those are already bound.
		const fallback = parameter.default;
		if (fallback === null || fallback === undefined) return null;
		next.set(parameter.name.name, resolvedSubstitutionArgument(fallback, next));
	}
	return next;
}
