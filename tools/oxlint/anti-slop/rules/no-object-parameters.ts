...
/**
 * Unwraps the same three carriers `parameterAnnotation` does, so the reported
 * name is the binding rather than its source text.
 *
 * Without this a constructor property reported ``private readonly o`` — the
 * whole declaration, modifiers included — because the fallback stripped only a
 * trailing `: object`. The twin in `no-unknown-parameters` already unwrapped
 * all three; this copy had not.
 */
