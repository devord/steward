...
function widenedBinding(
  ...
  // LIMITATION: only the *initializer* form is detectable here —
  // `const x = y as unknown`. The declared form, `const x: unknown = y`, is
  // not reachable: oxlint's AST declares `BindingIdentifier.typeAnnotation`
  // as `null` (no binding node in the API carries a `TSTypeAnnotation`), so
  // there is no path to a const's written type. This read used to be
  // `declarator.id.typeAnnotation?.typeAnnotation`, which optional-chained a
  // `null` to `never` and silently contributed nothing. It is removed rather
  // than left in place, because dead code here reads as coverage the rule
  // does not have.
  ...
  // `broadKind` is non-null only when the initializer was an assertion, so
  // the null branch is unreachable in practice and kept for narrowing alone.
