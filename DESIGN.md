...
**In the lockup the glyph is sized against the word, not against the font
size.** `chromeGlyphBox` lands the bow's **ink** at `GLYPH_INK_CAP` (0.85) of
Geist Mono 600's cap band, and derives both box dimensions back out through the
crop so nothing letterboxes. Under 1 on purpose: the bow is a wide mark — 2.45:1
through the ink — so it takes its mass from its width before it takes any from
its height, and level with the caps it outweighs the seven letters it leads. The
box was a literal `1em` until it was measured, which reads as "one line tall"
but sizes the **crop** — the ink plus `GLYPH_AIR` on all four sides — so it drew
a bow 1.15× the cap height, and in the 240px rail that put the widest, darkest
object on the column above the nav it introduces. The ratio belongs to the
lockup, so it holds at every size and on every surface: the phone header and the
desktop rail carry one brand, and one proportion inside it.
