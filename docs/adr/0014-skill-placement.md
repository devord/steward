...
Routine skills are content; the steward repo is the platform. Mixing them
...
- **Steward (shared product repo)** — _contract skills only_:
...
- **Plugins repo** (`form-factory/plugins`) — shared/team routine skills
  ...
  no steward coupling.
- **Data repo** (`steward-data-<login>`, and the team repo) — private
...
has steward + the data repo checked out and the plugins repo installed.
No `source:` field, no custom loader.

## Considered options

- **Three-tier placement by readership (chosen).**
- **Content skills in steward** (status quo) — private skills are
  impossible (steward is team-visible) and team skills couple to product
  releases.
...
- Local runs launch Claude in the _data repo_ (ADR-0017), where the
  ...
  and the launchd agents pass `--add-dir <steward checkout>` (added dirs
  ...
  **not** mirrored into the plugins repo: they must ship atomically with
  ...
  Consequence: the app's copy-command one-liner runs from a steward
